...
class DataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def _support_onoff(self, deviceserial: str) -> bool:
        cap = self._data["capacity"].get(deviceserial)
        if not cap:
            return False
        # 只有明确支持隐私 or 场景开关的，才尝试
        return cap.get("support_privacy") == "1"
        ...
        for device in self._data["devicelistinfo"]:            
            ...
            if "on_off" in self._haswitchs:
                if self._support_onoff(device["deviceSerial"]):
                    ...
                    _LOGGER.debug(
                        "Skip GetDeviceonoff for %s (capacity not supported)",
                        device["deviceSerial"]
                    )
